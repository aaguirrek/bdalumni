...
import { type ZXingDecodeHints, type DecodeHints, type ZXingEncodeHints, type EncodeHints, type ZXingReadResult, type ReadResult, type ZXingWriteResult, type ZXingBinarizer, type ZXingCharacterSet, type ZXingContentType, type ZXingEanAddOnSymbol, type ZXingTextMode, type ZXingVector } from "./bindings/index.js";
...
export interface ZXingReaderModule extends ZXingBaseModule {
    ...
    readBarcodesFromImage(bufferPtr: number, bufferLength: number, zxingDecodeHints: ZXingDecodeHints): ZXingVector<ZXingReadResult>;
    readBarcodesFromPixmap(bufferPtr: number, imgWidth: number, imgHeight: number, zxingDecodeHints: ZXingDecodeHints): ZXingVector<ZXingReadResult>;
}
/**
 * @internal
 */
export interface ZXingWriterModule extends ZXingBaseModule {
    writeBarcodeToImage(text: string, zxingEncodeHints: ZXingEncodeHints): ZXingWriteResult;
}
/**
 * @internal
 */
...
export declare function readBarcodesFromImageFileWithFactory<T extends "reader" | "full">(zxingModuleFactory: ZXingModuleFactory<T>, imageFile: Blob | File, decodeHints?: DecodeHints): Promise<ReadResult[]>;
export declare function readBarcodesFromImageDataWithFactory<T extends "reader" | "full">(zxingModuleFactory: ZXingModuleFactory<T>, imageData: ImageData, decodeHints?: DecodeHints): Promise<ReadResult[]>;
export declare function writeBarcodeToImageFileWithFactory<T extends "writer" | "full">(zxingModuleFactory: ZXingModuleFactory<T>, text: string, encodeHints?: EncodeHints): Promise<import("./bindings/writeResult.js").WriteResult>;
export {};
